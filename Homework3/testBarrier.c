...
int main()
{
    ...
    int* myRange[2];    //각 프로세스의 점 생성 범위의 시작(mrRange[myrank][0])과 끝(myRange[myrank][1])
    const int START = 0, END = 1;   //myRange 인덱스
    int myrank, ncpus;
    int lx, ly, gap, Lx, Ly;    //lx, ly : 점 개수, gap : 간격, Lx, Ly : x, y의 최댓값
    int npoint; //전체 점의 개수
